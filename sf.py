...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='SpiderFoot: Open Source Intelligence Automation.')
    parser.add_argument('-d', '--debug', action='store_true', help='Enable debug output.')
    parser.add_argument('-l', help='Log directory.', metavar='DIR', type=str)
    parser.add_argument('-m', '--modules', help='Comma-separated list of modules to enable.', type=str)
    parser.add_argument('-M', '--module-info', help='Get module information.', type=str)
    parser.add_argument('-s', '--start', help='Target for the scan.', type=str)
    parser.add_argument('-t', '--types', help='Types of scan to run.', type=str)
    parser.add_argument('-u', '--usecase', help='Use case to run.', type=str)
    parser.add_argument('-T', '--template', help='Scan template to use.', type=str)
    parser.add_argument('-n', '--name', help='Name of the scan.', type=str)
    parser.add_argument('-o', '--output', help='Output format.', type=str)
    parser.add_argument('-f', '--filter', help='Event types to filter out.', type=str)
    parser.add_argument('-F', '--filtering', help='Build filter rules from previously run scans.', type=str)
    parser.add_argument('-x', '--noproxy', action='store_true', help='Disable proxy.', default=False)
    parser.add_argument('-S', '--suppress', action='store_true', help='Silent output.', default=False)
    parser.add_argument('-D', '--delete', action='store_true', help='Delete the scan.', default=False)
    parser.add_argument('-r', '--remote', help='Remote scan ID.', type=str)
    parser.add_argument('-H', '--headers', action='store_true', help='Include headers in output.', default=False)
    parser.add_argument('-v', '--version', action='store_true', help='Display Version')
    parser.add_argument('-nowebserver', action='store_true', help='Do not start the web server')
...
def load_modules(config: dict) -> bool:
    ...
    try:
        ...
        corr_dir = os.path.dirname(os.path.abspath(__file__)) + '/correlations/'
...
def start_scan(config: dict, args, loggingQueue: mp.Queue) -> None:
    ...
    try:
        ...
        log.info(f"Starting scan '{scanName}' (ID: {scanId}) for target '{target}'")
...
def validate_scan_arguments(args) -> bool:
    ...
    if args.r and (args.o and args.o not in ["tab", "csv"]):
        log.error("Remote fetching is only compatible with tab and csv output formats.")
        return False
...
def process_target(target: str) -> str:
    ...
    targetType = SpiderFootHelpers.targetTypeFromString(target)
    if not targetType:
        log.error(f"Invalid target '{target}'. Could not determine target type.")
        return ""
...
def prepare_modules(args, dbh, config: dict, targetType: str) -> List[str]:
    ...
    if args.u:
        ...
        for module in sorted(config['__modules__']):
            ...
            if usecase == 'passive' and 'passive' in config['__modules__'][module]['flags']:
                modlist.append(module)
                continue
                
            if usecase == 'investigate' and 'investigate' in config['__modules__'][module]['flags']:
                modlist.append(module)
                continue
                
            if usecase == 'footprint' and 'footprint' in config['__modules__'][module]['flags']:
                modlist.append(module)
                continue
...
def execute_scan(loggingQueue: mp.Queue, target: str, targetType: str, modlist: List[str], config: Dict[str, Any]):
    ...
    try:
        # Start the scan
        startSpiderFootScanner(loggingQueue, scanName, scanId, target, targetType, modlist, config)
    except Exception as e:
