...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Export SpiderFoot scan results to MISP format')
    ...
    parser.add_argument('-p', '--publish', help='Publish directly to MISP', action='store_true')
    parser.add_argument('-u', '--misp-url', help='MISP URL (required if publishing)')
    parser.add_argument('-k', '--misp-key', help='MISP API key (required if publishing)')
    parser.add_argument('-t', '--tlp', help='TLP tag (default: tlp:amber)', default='tlp:amber')
...
def main():
    ...
    if args.publish:
        try:
            ...
            if 'errors' in response:
                ...
                print(f"Successfully published event to MISP with ID: {response['Event']['id']}")
        except ImportError:
    ...
    # Print summary
    print(f"MISP event contains {len(misp_event.attributes)} attributes and {len(misp_event.objects)} objects")
