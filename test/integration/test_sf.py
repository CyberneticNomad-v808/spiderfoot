...
class TestSf(unittest.TestCase):
    ...
    def test_no_args_should_print_arg_l_required(self):
        ...
        self.assertIn(
            b"SpiderFoot requires -l <ip>:<port> to start the web server. Try --help for guidance.",
            out,
        )
    ...
    def test_run_scan_invalid_target_should_exit(self):
        invalid_target = "."
        out, err, code = self.execute(
            [sys.executable, "sf.py", "-s", invalid_target])
        self.assertIn(
            bytes(
                f"Could not determine target type. Invalid target: {invalid_target}",
                "utf-8",
            ),
            err,
        )
    ...
    def test_run_scan_with_modules_no_target_should_exit(self):
        out, err, code = self.execute(
            [sys.executable, "sf.py", "-m", ",".join(self.default_modules)]
        )
        self.assertIn(
            b"You must specify a target when running in scan mode", err)
        self.assertEqual(255, code)

    def test_run_scan_with_types_no_target_should_exit(self):
        out, err, code = self.execute(
            [sys.executable, "sf.py", "-t", ",".join(self.default_types)]
        )
        self.assertIn(
            b"You must specify a target when running in scan mode", err)
        self.assertEqual(255, code)
    ...
    def test_run_scan_with_invalid_module_should_run_scan_and_exit(self):
        ...
        out, err, code = self.execute(
            [sys.executable, "sf.py", "-m", module, "-s", "spiderfoot.net"]
        )
    ...
    def test_run_scan_with_invalid_type_should_exit(self):
        out, err, code = self.execute(
            [sys.executable, "sf.py", "-t", "invalid type", "-s", "spiderfoot.net"]
        )
