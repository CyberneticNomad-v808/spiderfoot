...
class TestSpiderFootTarget(unittest.TestCase):
    ...
    def test_matches(self, mock_ipnetwork, mock_ipaddress):
        ...
        self.assertTrue(target.matches("sub.example.com", includeParents=False, includeChildren=True))
        self.assertTrue(target.matches("com", includeParents=True, includeChildren=False))
        self.assertFalse(target.matches("sub.example.com", includeParents=False, includeChildren=False))
        self.assertFalse(target.matches("com", includeParents=False, includeChildren=False))
        ...
        self.assertTrue(human_target.matches("anything"))  # All matches return True for special targets
        
        username_target = SpiderFootTarget("johndoe", "USERNAME")
        self.assertTrue(username_target.matches("anything"))  # All matches return True for special targets
        
        bitcoin_target = SpiderFootTarget("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa", "BITCOIN_ADDRESS")
        self.assertTrue(bitcoin_target.matches("anything"))  # All matches return True for special targets
        
        phone_target = SpiderFootTarget("+12125551212", "PHONE_NUMBER")
        self.assertTrue(phone_target.matches("anything"))  # All matches return True for special targets
