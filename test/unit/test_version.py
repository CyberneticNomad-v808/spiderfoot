...
class TestVersion(unittest.TestCase):
    ...
    def test_version_format(self):
        ...
        self.assertTrue(re.match(pattern, __version__), f"Version {__version__} does not match semantic versioning")
    ...
    def test_version_components(self):
        ...
        self.assertEqual(len(components), 3, "Version should have major, minor, and patch components")
        
        # All components should be integers
        for component in components:
            self.assertTrue(component.isdigit(), f"Version component {component} is not a number")
        
        # Extract prerelease if present
        if '-' in __version__:
            prerelease = __version__.split('-')[1]
            self.assertTrue(len(prerelease) > 0, "Prerelease component should not be empty")
