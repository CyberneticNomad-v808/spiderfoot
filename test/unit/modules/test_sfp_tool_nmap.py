...
class TestModuleToolNmap(TestModuleBase):

    def setUp(self: 'TestModuleToolNmap') -> None:
    ...
    def tearDown(self: 'TestModuleToolNmap') -> None:
        """Clean up after each test."""
        super().tearDown()

    def test_opts(self: 'TestModuleToolNmap') -> None:
        """Test that opts and optdescs have same length."""
        module = sfp_tool_nmap()
        self.assertEqual(len(module.opts), len(module.optdescs))

    def test_setup(self: 'TestModuleToolNmap') -> None:
        """Test module setup."""
    ...
    def test_handleEvent_no_tool_path_configured_should_set_errorState(
        ...
        source_event = SpiderFootEvent(
            'ROOT', target_value, 'sfp_spider', None
        )
    ...
    @unittest.mock.patch(
        'modules.sfp_tool_nmap.os.path.isfile',
        return_value=True
    )
    @unittest.mock.patch('modules.sfp_tool_nmap.Popen')
    def test_handleEvent_local_success(
        self: 'TestModuleToolNmap',
        mock_popen: unittest.mock.MagicMock,
        mock_isfile: unittest.mock.MagicMock,
    ) -> None:
        """Test handleEvent succeeds with local nmap.

        Args:
            mock_popen: Mocked Popen class.
        ...
        event = SpiderFootEvent(
            'IP_ADDRESS', '1.2.3.4', 'test', None
        )
        module.handleEvent(event)
        self.assertTrue(module.notifyListeners.called)
        self.assertEqual(
            module.notifyListeners.call_args[0][0].eventType,
            'OPERATING_SYSTEM'
        )

    @unittest.mock.patch(
        'modules.sfp_tool_nmap.paramiko.SSHClient'
    )
    @unittest.mock.patch(
        'modules.sfp_tool_nmap.io.StringIO'
    )
    @unittest.mock.patch(
        'modules.sfp_tool_nmap.paramiko.RSAKey'
        '.from_private_key',
        return_value='pkey'
    )
    def test_handleEvent_remote_with_pasted_key(
        self: 'TestModuleToolNmap',
        mock_rsa: unittest.mock.MagicMock,
        mock_stringio: unittest.mock.MagicMock,
        mock_sshclient: unittest.mock.MagicMock,
    ) -> None:
        """Test handleEvent with remote nmap via pasted SSH key.

        Args:
            mock_rsa: Mocked RSAKey.from_private_key.
            mock_stringio: Mocked io.StringIO.
            mock_sshclient: Mocked paramiko.SSHClient.
        """
        sf = SpiderFoot(self.default_options)
        module = sfp_tool_nmap()
        module.setup(sf, {
        ...
        mock_ssh.connect.assert_called_with(
            '1.2.3.4', username='user', pkey='pkey',
    ...
    def test_handleEvent_remote_with_keyfile(
        self: 'TestModuleToolNmap',
        mock_sshclient: unittest.mock.MagicMock,
    ) -> None:
        """Test handleEvent with remote nmap via SSH key file.
