...
class TestSpiderFootAPICore(TestModuleBase):
    ...
    def setUp(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_clean_user_input_basic(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_config_class_instantiation(self: 'TestSpiderFootAPICore') -> None:
        """Test Config class can be instantiated."""
        from spiderfoot.api.dependencies import Config
        with patch('spiderfoot.api.dependencies.SpiderFootDb') as mock_db_class, \
             patch('spiderfoot.api.dependencies.SpiderFoot') as mock_sf_class:
    ...
    def test_get_app_config_function(self: 'TestSpiderFootAPICore') -> None:
        """Test get_app_config function."""
        import spiderfoot.api.dependencies as deps
        with patch('spiderfoot.api.dependencies.Config') as mock_config_class:
            mock_config = MagicMock()
            mock_config_class.return_value = mock_config

            # Reset global app_config
            original_config = deps.app_config
            original_testing = os.environ.get('TESTING_MODE')
            deps.app_config = None
            # Temporarily disable testing mode so get_app_config actually creates Config
            os.environ.pop('TESTING_MODE', None)

            try:
                result = deps.get_app_config()
                self.assertIsNotNone(result)
            finally:
                # Restore original config and testing mode
                deps.app_config = original_config
                if original_testing is not None:
                    os.environ['TESTING_MODE'] = original_testing
                else:
                    os.environ.pop('TESTING_MODE', None)

    def test_search_base_function(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_build_excel_function(self: 'TestSpiderFootAPICore') -> None:
        ...
        with patch('sfapi.openpyxl') as mock_openpyxl:
            ...
            try:
                self.sfapi.build_excel(test_data, columns)
    ...
    def test_websocket_manager_class(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_pydantic_models(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_fastapi_app_components(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_authentication_functions(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_logging_and_multiprocessing(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_version_and_constants(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_error_handling_components(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_fastapi_dependencies(self: 'TestSpiderFootAPICore') -> None:
    ...
    def test_utility_function_calls(self: 'TestSpiderFootAPICore') -> None:
