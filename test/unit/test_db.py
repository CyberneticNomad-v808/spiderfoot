...
class TestSpiderFootDb(unittest.TestCase):
    ...
    def test_create_schema(self):
        ...
        with sqlite3.connect(self.temp_db_file) as conn:
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='tbl_scan_instance'")
            self.assertIsNotNone(cursor.fetchone())
            
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='tbl_scan_results'")
            self.assertIsNotNone(cursor.fetchone())
            
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='tbl_event_types'")
            self.assertIsNotNone(cursor.fetchone())
    ...
    def test_scan_instance_list(self):
        ...
        self.db.scanInstanceCreate("test-scan-id-2", "Test Scan 2", "example.org")
    ...
    def test_scan_instance_delete(self):
        ...
        self.db.scanInstanceCreate(delete_scan_id, "Scan to Delete", "example.org")
    ...
    def test_scan_log_event(self):
        ...
        # Log an event
        self.db.scanLogEvent(self.test_scan_id, "TEST", "Test message", "TestModule")
    ...
    def test_scan_event_store_and_retrieve(self):
        ...
        # Create a test event
        source_event = SpiderFootEvent("ROOT", self.test_scan_target, "TEST", "")
        evt = SpiderFootEvent("INTERNET_NAME", "example.com", "test_module", source_event)
