...
class SpiderFoot:
        ...
        try:
            ...
            for x in ext.value:
                if isinstance(x, cryptography.x509.DNSName):
                    ret['altnames'].append(x.value.lower().encode(
                        'raw_unicode_escape').decode("ascii", errors='replace'))
        except Exception as e:
    ...
    def removeUrlCreds(self, url: str) -> str:
        ...
        ret = url
        for pat in pats:
            ret = re.sub(pat, pats[pat], ret, count=0, flags=re.IGNORECASE)
    ...
    def cveInfo(self, cveId: str, sources: str = "circl,nist") -> (str, str):
        ...
        for source in sources:
            ...
            if not jsondata:
                ...
                if source == "nist":
                    ret = self.fetchUrl(
                        f"https://services.nvd.nist.gov/rest/json/cve/1.0/{cveId}", timeout=5)
                if source == "circl":
                    ret = self.fetchUrl(
