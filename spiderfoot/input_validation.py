...
class InputValidator:
    ...
    def _fallback_sanitize_html(cls, content: str) -> str:
        ...
        dangerous_tags = ['script', 'object', 'embed', 'form', 'input',
                         'iframe', 'style']
        ...
        if not re.search(f'<({allowed_pattern})\\b[^>]*>', content,
                        re.IGNORECASE):
    ...
    @classmethod
    def sanitize_module_options(cls,
        ...
        for key, value in options.items():
            ...
            if isinstance(value, str):
                if 'api_key' in key.lower() or 'password' in key.lower():
                    ...
                    sanitized[key] = cls.escape_html(value)[:1000]
    ...
    def validate_json_input(cls, data: Any) -> bool:
        ...
        Returns:
            True if all values are safe, False if any are potentially
            malicious
        """
