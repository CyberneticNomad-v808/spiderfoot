...
class SpiderFootMCPClient:
    ...
    def _load_report_templates(self) -> Dict[str, dict]:
        ...
        return {
            'threat_assessment': {
                ...
                'description': (
                    'Comprehensive threat assessment based on OSINT findings'
                ),
                'sections': [
                    'executive_summary',
                    'threat_landscape',
            ...
            'infrastructure_analysis': {
                ...
                'description': (
                    'Analysis of target infrastructure and security posture'
                ),
                'sections': [
    ...
    async def generate_cti_report(
        ...
        try:
            self.log.info(
                f"Generating CTI report for workspace {workspace.workspace_id}"
            )
            ...
            template = self.report_templates.get(
                report_type,
                self.report_templates['threat_assessment']
            )
            ...
            self.log.info(
                f"CTI report generated successfully for "
    ...
    async def _prepare_workspace_data(
        self,
        workspace: SpiderFootWorkspace
    ) -> Dict[str, Any]:
    ...
    def _is_threat_indicator(self, event_type: str) -> bool:
        ...
        threat_types = [
            ...
            'BLACKLISTED_IPADDR', 'MALICIOUS_AFFILIATE',
            'THREAT_INTELLIGENCE', 'VULNERABILITY_CVE_HIGH',
    ...
    def _is_infrastructure_data(self, event_type: str) -> bool:
        ...
        infra_types = [
            'IP_ADDRESS', 'DOMAIN_NAME', 'INTERNET_NAME',
            'NETBLOCK_OWNER', 'WEBSERVER_TECHNOLOGY',
            'OPERATING_SYSTEM', 'SSL_CERTIFICATE_ISSUED'
        ]
    ...
    def _is_data_leakage(self, event_type: str) -> bool:
        ...
        leak_types = [
            'EMAILADDR', 'PHONE_NUMBER', 'HUMAN_NAME',
            'PHYSICAL_ADDRESS', 'ACCOUNT_EXTERNAL_OWNED',
            'LEAKSITE_CONTENT', 'DARKWEB_MENTION'
        ]
    ...
    async def _prepare_mcp_request(
        ...
        base_prompt = f"""
        You are a cybersecurity threat intelligence analyst. Analyze the
        provided OSINT data and generate a comprehensive {template['name']}.
        ...
        3. Clear, professional language suitable for technical and
    ...
    async def _save_report_to_workspace(
        ...
        # Save full report separately (could be stored in files or
        # separate table)
        ...
        self.log.info(
            f"CTI report {report['report_id']} saved to workspace "
...
class CTIReportExporter:
    ...
    def export_report(
        ...
        if not output_path:
            ...
            output_path = (
                f"cti_report_{report['report_id']}_{timestamp}.{format}"
            )
    ...
    def _export_html(
        ...
        <head>
            ...
            <style>
                ...
                .finding {{ background: #f5f5f5; padding: 15px;
                ...
                .recommendations {{ background: #e3f2fd; padding: 20px;
                                   border-radius: 5px; }}
            </style>
        </head>
        <body>
    ...
    def _export_pdf(
        ...
        self.log.warning(
            f"PDF export not implemented. HTML version created at {html_path}"
        )
        self.log.warning(
            "Consider using wkhtmltopdf or similar tool to convert HTML to PDF"
        )
    ...
    def _export_docx(
        ...
        self.log.warning(
            f"DOCX export not implemented. HTML version created at {html_path}"
        )
        self.log.warning(
            "Consider using pandoc or python-docx to convert HTML to DOCX"
        )
