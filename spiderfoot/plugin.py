...
class SpiderFootPlugin:
    ...
    def __init__(self):
        ...
        # Initialize _listenerModules attribute
    ...
    def info(self, *args, **kwargs) -> None:
        """For logging. A wrapper around logging.info() that adds the scanId
        to LogRecord.
    ...
    def getScanId(self) -> str:
        ...
        if not self.__scanId__:
            raise TypeError(
                "Module called getScanId() but no scanId is set."
            )
    ...
    def getTarget(self) -> str:
        ...
        if not self._currentTarget:
            raise TypeError(
                "Module called getTarget() but no target is set."
            )

        return self._currentTarget

    def setOutputFilter(self, types) -> None:
        """Set output filter for events produced by this module.
    ...
    def tempStorage(self) -> dict:
        ...
        Todo:
            Move all module state to use this, which then would enable a
            scan to be paused/resumed.
    ...
    def checkForStop(self) -> bool:
        ...
        if (self.outgoingEventQueue is not None and
                self.incomingEventQueue is not None):
    ...
    def running(self) -> bool:
        ...
        """
        module_name = getattr(self, '__name__', self.__class__.__name__)
        task_name = f"{module_name}_threadWorker"
    ...
    def log(self):
        ...
        if self._log is None:
            ...
            logger_name = getattr(self, '__name__', self.__class__.__name__)
            self._log = logging.getLogger(f"spiderfoot.{logger_name}")
    ...
    def asdict(self) -> dict:
        ...
        Returns:
            dict: Plugin information as dictionary
        """
        return {
    ...
    def threadPool(self, *args, **kwargs):
        """Create and return a SpiderFootThreadPool instance.

        Args:
            *args: Arguments passed to SpiderFootThreadPool
            **kwargs: Keyword arguments passed to SpiderFootThreadPool

        Returns:
            SpiderFootThreadPool: New thread pool instance
        """
    ...
    def poolExecute(self, callback, *args, **kwargs) -> None:
        ...
        """
        module_name = getattr(self, '__name__', self.__class__.__name__)
        if module_name.startswith('sfp__stor_'):
            callback(*args, **kwargs)
        else:
            task_name = f"{module_name}_threadWorker"
            self.sharedThreadPool.submit(
                callback, *args, taskName=task_name,
                maxThreads=self.maxThreads, **kwargs)
    ...
    def setDbh(self, dbh) -> None:
        """Used to set the database handle, which is only to be used by
        modules in very rare/exceptional cases (e.g. sfp__stor_db)
    ...
    def registerListener(self, listener) -> None:
        """Listener modules which will get notified once we have data for
        them to work with.
    ...
    def notifyListeners(self, sfEvent) -> None:
        ...
        if (self.__outputFilter__ and
                eventName not in ['ROOT', self.getTarget().targetType,
                                  self.__outputFilter__]):
            return

        storeOnly = False  # Under some conditions, only store and don't
                           # notify
        ...
        while prevEvent is not None:
            if (prevEvent.sourceEvent is not None and
                    ...
                    prevEvent.sourceEvent.data.lower() ==
                    eventData.lower()):
                storeOnly = True
                break
        ...
        # otherwise, call other modules directly
            ...
            for listener in self._listenerModules:
                if (eventName not in listener.watchedEvents() and
                ...
                try:
                    ...
                    self.sf.error(
                        f"Module ({listener.__module__}) encountered "
                        f"an error: {e}")
                    # set errorState
    ...
    def watchedEvents(self) -> list:
        """What events is this module interested in for input. The format is
        a list of event types that are applied to event types that this
        module wants to be notified of, or * if it wants everything.
    ...
    def producedEvents(self) -> list:
        """What events this module produces This is to support the end user
        in selecting modules based on events produced.
    ...
    def start(self) -> None:
        ...
        if not (self.incomingEventQueue and self.outgoingEventQueue):
            ...
            module_name = getattr(self, '__name__', 'unknown')
            error_msg = (
                f"Module {module_name} cannot start: queues not "
                f"initialized (incoming="
                f"{self.incomingEventQueue is not None}, outgoing="
                f"{self.outgoingEventQueue is not None})"
            )
        ...
        if hasattr(self, 'sf') and self.sf:
            module_name = getattr(self, '__name__', 'unknown')
            self.sf.debug(f"Starting module {module_name} thread")
    ...
    def threadWorker(self) -> None:
        """Worker thread for processing module events and tasks.

        Handles incoming events from queue and executes handleEvent callbacks
        in a dedicated thread. Manages database connections and error handling
        for the module.
        """
        try:
            ...
            while not self.checkForStop():
                ...
                if sfEvent == 'FINISHED':
                    ...
                    self.sf.debug(
                        f"{module_name}.threadWorker() got \"FINISHED\" "
                        f"from incomingEventQueue.")
                    ...
                    self.sf.debug(
                        f"{module_name}.threadWorker() got event, "
                        f"{sfEvent.eventType}, from incomingEventQueue.")
                    self.poolExecute(self.handleEvent, sfEvent)
        except KeyboardInterrupt:
            module_name = getattr(self, '__name__', self.__class__.__name__)
            self.sf.debug(f"Interrupted module {module_name}.")
            self._stopScanning = True
        except Exception as e:
            import traceback
            module_name = getattr(self, '__name__', self.__class__.__name__)
            ...
            # set errorState
            module_name = getattr(self, '__name__', self.__class__.__name__)
            self.sf.debug(f"Setting errorState for module {module_name}.")
            self.errorState = True
            # clear incoming queue
            if self.incomingEventQueue:
                module_name = getattr(
                ...
                self.sf.debug(
                    f"Emptying incomingEventQueue for module {module_name}.")
                with suppress(queue.Empty):
                    while 1:
                        self.incomingEventQueue.get_nowait()
    ...
    def setSharedThreadPool(self, sharedThreadPool) -> None:
        ...
        Args:
            sharedThreadPool: Thread pool object to use for task execution
        """
