...
class RateLimiter:
    ...
    def __init__(self, redis_client=None, redis_host='localhost',
    ...
    def _get_rate_limit_key(self, client_id: str, limit_type: str,
                           window_type: str = 'main') -> str:
        ...
        return (
            f"rate_limit:{limit_type}:{window_type}:{client_hash}:{window}"
        )

    def _check_redis_limit(self, client_id: str,
                          limit_type: str) -> Tuple[bool, Dict]:
    ...
    def _check_memory_limit(self, client_id: str,
                           limit_type: str) -> Tuple[bool, Dict]:
        ...
        if allowed:
            ...
            self._memory_store[client_id] = [
                t for t in timestamps if t > burst_cutoff
            ]
        ...
        info = {
            ...
            'main_remaining': max(
                0,
                config['requests'] - len(recent_main) - (1 if allowed else 0)
            ),
            ...
            'burst_remaining': max(
                0,
                config['burst'] - len(recent_burst) - (1 if allowed else 0)
            ),
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if not allowed:
                    retry_after = (
                        min(info['main_reset'], info['burst_reset']) -
                        int(time.time())
                    )
                    response = jsonify({
                        ...
                        'message': (
                            ...
                            f'{info["main_limit"]} per hour, '
                            f'{info["burst_limit"]} per minute'
                        ),
