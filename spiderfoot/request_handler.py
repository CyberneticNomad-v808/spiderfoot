...
class RequestHandler:
    ...
    def _create_session(self) -> requests.Session:
        ...
        if self.opts.get('_socks1type') and self.opts.get('_socks2addr') and self.opts.get('_socks3port'):
            ...
            self.log.debug(f"Using proxy: {self.sanitize_url_for_logging(proxy_url)}")
            session.proxies = {
                'http': proxy_url,
                'https': proxy_url
            }
    ...
    def get_user_agent(self) -> str:
        ...
        """
        user_agent = self.opts.get('_useragent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:62.0) Gecko/20100101 Firefox/62.0')
        ...
        if isinstance(user_agent, str) and user_agent.startswith('@'):
            user_agents = self._load_user_agents_from_file_or_url(user_agent[1:])
            if user_agents:
        ...
        try:
            if source.startswith(('http://', 'https://')):
                ...
                if response and response.get('content'):
                    user_agents = response['content'].decode('utf-8').splitlines()
            else:
                with open(source, 'r') as f:
                    user_agents = [line.strip() for line in f]
                
            # Remove empty lines and comments
            user_agents = [ua for ua in user_agents if ua and not ua.startswith('#')]
            
            self.log.info(f"Loaded {len(user_agents)} user agents from {source}")
        except Exception as e:
            self.log.error(f"Failed to load user agents from {source}: {e}")
            
        return user_agents if user_agents else ['Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:62.0) Gecko/20100101 Firefox/62.0']
    ...
    def sanitize_url_for_logging(self, url: str) -> str:
        ...
        try:
            ...
            if parsed_url.query:
                sensitive_params = ['api_key', 'apikey', 'key', 'password', 'token', 'auth', 'secret']
    ...
    def fetch_url(
        ...
        self.log.debug(f"Fetching {self.sanitize_url_for_logging(url)} with method {method}")
        ...
        while tries <= retry_times:
            try:
                ...
                break
            except (requests.RequestException, urllib.error.URLError, ssl.SSLError) as e:
                exception = e
                tries += 1
                if tries <= retry_times:
                    self.log.debug(f"Request to {self.sanitize_url_for_logging(url)} failed: {e}. Retrying ({tries}/{retry_times})...")
                    time.sleep(retry_pause)
                else:
                    self.log.error(f"Request to {self.sanitize_url_for_logging(url)} failed after {retry_times} retries: {e}")
    ...
    def download_file(self, url: str, local_path: str) -> bool:
        ...
        try:
            ...
            if not response or not response.get('content'):
                self.log.error(f"Failed to download from {self.sanitize_url_for_logging(url)}: No content received")
                return False
                
            if response.get('code') != 200:
                self.log.error(f"Failed to download from {self.sanitize_url_for_logging(url)}: HTTP {response.get('code')}")
                return False
                
            with open(local_path, 'wb') as f:
                f.write(response['content'])
                
            self.log.debug(f"Downloaded {self.sanitize_url_for_logging(url)} to {local_path}")
            return True
            
        except Exception as e:
            self.log.error(f"Failed to download from {self.sanitize_url_for_logging(url)}: {e}")
            return False
