...
def handle_exception(
    exception: Exception, module_name: str, fatal: bool = False
) -> None:
    ...
    if fatal:
        logger.critical(
            f"FATAL ERROR ({error_type}): {error_message}", exc_info=True)
        sys.exit(1)
    else:
...
def api_error_handler(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger = get_module_logger(module_name)
            logger.error(
                f"API Error in {func.__name__}: {str(e)}", exc_info=True)
