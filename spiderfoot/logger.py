...
class SpiderFootSqliteLogHandler(logging.Handler):
    ...
    def process_log_batch(self):
        ...
        if self.dbh is None:
            # If still no database handle, put items back in batch and log error
            print(f"[ERROR] Cannot create database handle for logging, retaining {len(batch)} log entries")
            self.batch = batch + self.batch  # Prepend failed batch to current batch
            return

        logResult = self.dbh.scanLogEvents(batch)
        if logResult is False:
            print(f"[WARNING] Log batch insert failed, attempting to recreate database handle")
            # Try to recreate database handle if insert failed
            self.makeDbh()
            if self.dbh:
                retry_result = self.dbh.scanLogEvents(batch)
                if retry_result is False:
                    print(f"[ERROR] Log batch insert failed after retry, losing {len(batch)} log entries")
            else:
                print(f"[ERROR] Cannot recreate database handle, losing {len(batch)} log entries")
    ...
    def makeDbh(self) -> None:
        ...
        try:
            ...
            elif hasattr(self.dbh, 'db_type') and self.dbh.db_type == 'sqlite':
                # For SQLite, test with a simple query
                with self.dbh.dbhLock:
                    self.dbh.dbh.execute("SELECT 1")
        except Exception as e:
            print(f"[ERROR] Failed to create database handle for logging: {e}")
            self.dbh = None
